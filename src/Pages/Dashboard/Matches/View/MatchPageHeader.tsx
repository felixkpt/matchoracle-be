...
const MatchPageHeader = ({ game, homeTeam, awayTeam, homeTeamRecentResults, awayTeamRecentResults }: Props) => {
  ...
  return (
    <div className='header-title shadow-sm p-3 rounded mb-3 row justify-content-between cursor-default'>
      ...
      <div className='col-12 col-md-6 col-lg-7 order-1 order-md-1 header-title shadow-sm border p-2 rounded mb-3 row text-center bg-light z-50'>
        <div className="competition-head d-flex flex-column align-items-center mb-2">
          <NavLink to={`/dashboard/competitions/view/${game.competition.id}`} className={`btn-link`}>
            <div className="p-1">
              {
        ...
        <div>
          <h4><NavLink to={`/dashboard/teams/view/${homeTeam.id}`} className="btn-link">{homeTeam.name}</NavLink> <span className="mx-2">vs</span> <NavLink to={`/dashboard/teams/view/${awayTeam.id}`} className="btn-link">{awayTeam.name}</NavLink></h4>
