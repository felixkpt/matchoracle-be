...
const CreateOrUpdate = () => {
  ...
  useEffect(() => {
    ...
    if (id) {
      getPost(`dashboard/posts/view/${id}`).then(res => {
  ...
  useEffect(() => {
    ...
    if (post) {
      ...
      getPost('/dashboard/settings/picklists/statuses/post?all=1').then((res) => {
    ...
    const handleAjaxPostDone = (event: any) => {
      if (event?.detail) {
        ...
        if (elementId === 'posts-form' && results) {
          navigate('/dashboard/posts/view/' + results.id + '/edit');
        }
      }
    };
  ...
  return (
    <div>
      <PageHeader title={`${id ? 'Edit Post #' + id : 'Create Post'}`} listUrl='/dashboard/posts' />
      <div className='card'>
        <div className="card-body">
          <form key={key} id={`posts-form`} onSubmit={(e) => publish('ajaxPost', e, { image: files[0] })}
            action-url={
              post
                ? `/dashboard/posts/view/${post.id}`
                : 'dashboard/posts'
            }
                      ...
                      init={{
                        ...
                        file_picker_callback: function (cb, value, meta) {
                          ...
                          input.onchange = function () {
                            ...
                            doPost('/dashboard/file-repo/upload-image', formData).then((results) => {

                              if (results) {
                                const { data, token } = results

                                cb(baseURL('/dashboard/file-repo/' + data.path + '?token=' + token), { title: data.caption, class: 'asas' });
                              }
