...
const TeamMatchesCard = ({ team, teamGames, context }: Props) => {
                                    ...
                                    <NavLink key={game.id} to={`/admin/matches/view/${game.id}`} className={`text-decoration-none text-dark`}>
                                        <div className='row'>
                                            ...
                                            <div className="col-5 d-flex flex-column">
                                                <div className='col-5 text-nowrap d-flex align-items-center justify-content-between gap-1'><span><img className='symbol-image-xm' src={home_team.crest} alt="" /></span><span className={`text-nowrap ${winningSide === 'h' ? 'fw-medium' : ''}`}>{Composer.team(home_team, 'short')}</span></div>
                                                <div className='col-5 text-nowrap d-flex align-items-center justify-content-between gap-1'><span><img className='symbol-image-xm' src={away_team.crest} alt="" /></span><span className={`text-nowrap ${winningSide === 'a' ? 'fw-medium' : ''}`}>{Composer.team(away_team, 'short')}</span></div>
                                            </div>
