...
const AutoModal: React.FC<ModalProps> = ({ modelDetails, record, actionUrl, modalSize, id, setKey, listSources, listSelects }) => {
    ...
    if (modelDetails && modelDetails.tableId) {
    ...
    useEffect(() => {
        if (modelDetails && Object.keys(modelDetails).length > 0) {
            setModelName(modelDetails?.model_name || undefined);
        }
    }, [modelDetails]);
                    ...
                    <div className="modal-content">
                        {modelDetails ?
                            <div>
                                ...
                                <AutoModalBody modelDetails={modelDetails} record={record} modalSize={modalSize} setKey={setKey} listSources={listSources} listSelects={listSelects} setComputedSize={setComputedSize} />
                            </div>
