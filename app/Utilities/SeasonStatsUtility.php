    ...
    public function updateSeasonStats(Season $season): ?Season
    {
        try {
            return DB::transaction(function () use ($season) {
                ...
                $season = Season::where('id', $season->id)->lockForUpdate()->first();
                $counts = $season->games()
                    ...
                    ', [
                        GameLastAction::STATUS_PENDING,
                        GameLastAction::STATUS_FETCHED,
                        GameLastAction::STATUS_MISSING,
                        GameLastAction::STATUS_PENDING,
                        GameLastAction::STATUS_FETCHED,
                        GameLastAction::STATUS_MISSING,
                        GameLastAction::STATUS_PENDING,
                ...
                // Update season in DB inside the transaction
                $season->update($counts);

                return $season->refresh();
            });
        } catch (Exception $e) {
            Log::error("Failed to update stats for Season [ID: {$season->id}, Title: {$season->title}] - {$e->getMessage()}", [
                'season_id' => $season->id,
