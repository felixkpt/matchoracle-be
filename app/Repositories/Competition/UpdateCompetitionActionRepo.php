    ...
    public function updateAction($competitionId, $action)
    {
        ...
        switch ($action) {
            case 'seasons_last_fetch':
                dispatch(new SeasonsHandlerJob('fetch', $ignoreTiming, $competitionId));
                break;

            case 'standings_recent_results_last_fetch':
                dispatch(new StandingsHandlerJob('recent_results', $ignoreTiming, $competitionId));
                break;

            case 'standings_historical_results_last_fetch':
                dispatch(new StandingsHandlerJob('historical_results', $ignoreTiming, $competitionId));
                break;
            case 'matches_recent_results_last_fetch':
                // Dispatch the job for recent results
                dispatch(new MatchesHandlerJob('recent_results', $ignoreTiming, $competitionId));
                break;

            case 'matches_historical_results_last_fetch':
                // Dispatch the job for historical results
                dispatch(new MatchesHandlerJob('historical_results', $ignoreTiming, $competitionId));
                break;

            case 'matches_fixtures_last_fetch':
                // Dispatch the job for fixtures
                dispatch(new MatchesHandlerJob('fixtures', $ignoreTiming, $competitionId));
                break;

            case 'matches_shallow_fixtures_last_fetch':
                // Dispatch the job for shallow fixtures
                dispatch(new MatchesHandlerJob('shallow_fixtures', $ignoreTiming, $competitionId));
                break;

            case 'match_recent_results_last_fetch':
                // Dispatch job for recent results
                dispatch(new MatchHandlerJob('recent_results', $ignoreTiming, $competitionId));
                break;

            case 'match_historical_results_last_fetch':
                // Dispatch job for historical results
                dispatch(new MatchHandlerJob('historical_results', $ignoreTiming, $competitionId));
                break;

            case 'match_fixtures_last_fetch':
                // Dispatch job for fixtures
                dispatch(new MatchHandlerJob('fixtures', $ignoreTiming, $competitionId));
                break;

            case 'match_shallow_fixtures_last_fetch':
                // Dispatch job for shallow fixtures
                dispatch(new MatchHandlerJob('shallow_fixtures', $ignoreTiming, $competitionId));
                break;

            case 'predictions_last_train':
                dispatch(new TrainPredictionsHandlerJob('train', $ignoreTiming, $competitionId));
                break;

            case 'predictions_last_done':
                dispatch(new PredictionsHandlerJob('prediction', $ignoreTiming, $competitionId));
                break;

            case 'stats_last_done':
                dispatch(new CompetitionStatisticsJob('stats', $ignoreTiming, $competitionId));
                break;

            case 'predictions_stats_last_done':
                dispatch(new CompetitionPredictionStatisticsJob('stats', $ignoreTiming, $competitionId));
                break;
