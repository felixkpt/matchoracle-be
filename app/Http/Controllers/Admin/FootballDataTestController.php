...
class FootballDataTestController extends Controller
{
    ...
    function index()
    {
        ...
        try {
            ...
            $seasonInit = $season = $competition->seasons()->whereYear('start_date', $season)->first();
            $seasonCompetition = $season->competition->competition->gameSources()->where(function ($q) use ($id) {
                $q->where('game_source_id', $this->api->sourceId)->where('competition_id', $id);
            })->first()->pivot;
            
            dd($seasonCompetition);
            ...
            foreach ($matchesData as $match) {
                ...
                if ($homeTeam && $awayTeam) {
                    ...
                    $this->saveGame($match, $competition, $season, $country, $homeTeam, $awayTeam, $saved, $updated);
                } else {
    ...
    private function saveGame($match, $country, $competition, $season, $homeTeam, $awayTeam, &$saved, &$updated)
    {
        $seasonData = $match->season;
        // let us get season 
        if (!$season)
            $season = app(Seasons::class)->updateOrCreate($seasonData, $country, $competition, $is_current = false);
    ...
    function storeScores($game, $score)
    {
        ...
        $game->game_score_id = $gameScore->id;
        $game->save();
    }
}
