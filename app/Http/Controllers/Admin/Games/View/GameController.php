...
namespace App\Http\Controllers\Admin\Games\View;
...
class GameController extends Controller
{
    ...
    {
        $this->table = request()->year . '_games';
        $this->id = request()->id;

        $this->gameRepository = autoModel($this->table);
    }


    public function index()
    {
        $builder = $this->gameRepository->where($this->table.'.id', $this->id)
        ->join('teams as hteam', $this->table.'.home_team_id', 'hteam.id')
        ->join('teams as ateam', $this->table.'.away_team_id', 'ateam.id')
        ->select($this->table.'.*', 'hteam.name as home_team', 'ateam.name as away_team');
