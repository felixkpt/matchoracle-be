...
class AdminController extends Controller
{
    ...
    public function advancedStats()
    {
        ...
        $matchesJobLogs = $this->getJobLogsStats(MatchesJobLog::class, $today, 'updated_matches');
        $resultsMatchJobLogs = $this->getMatchJobLogsStats('results', $today);
        $historicalResultsMatchJobLogs = $this->getMatchJobLogsStats('historical_results', $today);
        $fixturesMatchJobLogs = $this->getMatchJobLogsStats('fixtures', $today);
    ...
    private function getMatchJobLogsStats($task, $date)
    {
        $selects = 'SUM(job_run_counts) as total_job_run_counts, SUM(competition_run_counts) as total_competition_run_counts, SUM(fetch_run_counts) as total_fetch_run_counts, SUM(fetch_success_counts) as total_fetch_success_counts, SUM(fetch_failed_counts) as total_fetch_failed_counts, SUM(updated_matches_counts) as total_updated_items_counts';

        return [
            'all' => MatchJobLog::where('task', $task)->selectRaw($selects)->first(),
            'today' => MatchJobLog::where('task', $task)->whereDate('date', $date)->selectRaw($selects)->first(),
        ];
    }
